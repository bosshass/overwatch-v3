...
export default function SchedulerModal({ job, actor, onClose, onBooked }) {
  ...
  const days = new Set(chosen.map(id => picked[id].day || 1));
      ...
      <div style={S.modal} onClick={e => e.stopPropagation()}>
        ...
        {chosen.length > 0 && (
          <div style={S.summary}>
            ...
            {days.size > 1 && ` · ${days.size} days`}
            <div style={S.cal}>
              → {chosen.length > 1 ? 'multi-tech calendar' : 'technician calendar'}
            </div>
          </div>
        )}
        ...
        <div style={S.note}>
          Booking is the only way into Scheduled. Nothing writes to Google
          Calendar yet — assignments are recorded in the database only.
        </div>
      </div>
    </div>
  );
